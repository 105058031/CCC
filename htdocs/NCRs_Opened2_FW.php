...
FROM {$schem}.{$tab}
WHERE {$CL} = '".$argument1."'
AND {$PD} < CAST(current_date-CAST(date_part('dow',current_date+1) as INTEGER) as TIMESTAMP)
AND {$PD} > CAST(current_date-CAST(date_part('dow',current_date+1) as INTEGER)-2 as TIMESTAMP)
AND {$STS} <> 'Pending'
ORDER BY {$PD} ASC
;";
}
...
where 
{$CL} = '".$argument1."'
and {$PD} < CAST((current_date) as TIMESTAMP)
