...
ELSE 1 END as {$fr} 
FROM 
		(SELECT ROW_NUMBER() OVER (partition by {$Order} order by {$Op}, {$Day}) as {$Seq}, *  from {$schem}.{$tab1}
		WHERE {$OStat} = 'REL') {$Prim}
		WHERE {$Prim}.{$Seq} = 1
		AND (Substring({$Prim}.{$wc},1,3)='". $argument1 ."_' or Substring({$Prim}.{$wc},1,2)='HP' or Substring({$Prim}.{$wc},1,3)='SF_') and {$Prim}.{$plant}='". $argument2 ."'
