...
//$dow =1 ;
if (($dow < 2) OR ($dow > 5)) {
$sql = "SELECT a.*, b.* FROM (SELECT * FROM {$schem}.{$tab} WHERE char_length({$schem}.{$tab}.{$field53}) > 5) a
	...
	WHERE {$field50} = CAST(current_date-CAST(date_part('dow',current_date+1) as INTEGER)-2 as TIMESTAMP)
...
WHERE a.{$field50} = CAST(current_date-CAST(date_part('dow',current_date+1) as INTEGER)-2 as TIMESTAMP)
AND a.{$field62} > 0
...
$jsonData=array();
    ...
    die('  '. print_r($db->errorInfo()));
                ...
                {
                                 $jsonData[] = array('WorkCenter'=>$row['Work_Center'],'Date'=>$row['Date'],'Time'=>$row['Time'],'Order'=>$row['Order'],'Operation'=>$row['Operation No'],'Emp'=>$row['Emp'],'Material'=>$row['Material'], 'Rework'=>$row['Rework'],
