...
$sql = ";SELECT hgrd.[Posting Date],hgrd.Work_Center,hgrd.[Order], hgrd.Sum_Lab, hgrd.Material, SUM(hgrd.Sum_std) as STD FROM (SELECT k.*, (g.[Labor]/g.[Base Qty]) as Sum_Std FROM 
(Select h.[Posting Date]
      ,j.[Order]
	  ,j.[Work_Center]
      ,(sum(j.[Labor])/Max(h.[Qty in Un# of Entry])) as Sum_Lab    
	  ,h.[Material]
	  , h.[Movement Type]
  FROM [Searing_Dragon_Hell_Fire].[dbo].[ZP03] j
  left join [Searing_Dragon_Hell_Fire].[dbo].[MB51] h
  on h.[Order]=convert(float,j.[Order])
  WHERE j.[Order] < 59999999
