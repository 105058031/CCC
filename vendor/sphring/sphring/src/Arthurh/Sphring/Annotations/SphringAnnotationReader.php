...
class SphringAnnotationReader implements Reader
{
    ...
    public function initReader()
    {
		...
		if (!is_file($file)) {
            $file = trim($this->sphring->getRootProject(),"htdocs") . DIRECTORY_SEPARATOR . SphringComposerEnum::AUTLOADER_FILE;
        }
		if (!is_file($file)) {
            $file = trim($this->sphring->getRootProject(),"htdocs")  . SphringComposerEnum::AUTLOADER_FILE;
        }
        if (!is_file($file)) {
            throw new SphringAnnotationException("\n Can't find autoloader for annotation reading. \n Project Root : ".$this->sphring->getRootProject()." \n Context Root : " . $this->sphring->getContextRoot() . " \n Composer Dirname : " . dirname($this->composerManager->getComposerLockFile()) . " \n Server Doc Root: " . $_SERVER['CONTEXT_DOCUMENT_ROOT'] ." \n ComposerEnum: " . SphringComposerEnum::AUTLOADER_FILE . " \n Left trim : " . trim($this->sphring->getRootProject(),"htdocs") . " \n");
        }
        $loader = require $file;
